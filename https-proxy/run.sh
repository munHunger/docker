...
if [ -z ${renew+x} ]; then
  ...
  if [ -z ${distinct+x} ]; then
    ...
    certbot certonly --verbose --noninteractive --standalone --agree-tos --email="${email}" -d "${domains}"; else

    IFS=',' read -ra ADDR <<< "$domains"
    for domain in "${ADDR[@]}"; do
        certbot certonly --verbose --noninteractive --standalone --agree-tos --email="${email}" -d "${domain}"
    done
